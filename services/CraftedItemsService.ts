...
export const updateCraftedItemsWithRecipeId = async (professionSkillTrees: ProfessionSkillTree[]) => {
    ...
    const writeOperationsByProfession = recipesByProfession.map((professionRecipes) => professionRecipes.map<AnyBulkWriteOperation<ICraftingData>>((recipe) => ({ updateOne :
            {
                "filter": {
    ...
    return Promise.all(writeOperationsByProfession.map((writeOperations) => craftedItemCollection.bulkWrite(writeOperations)))
}
